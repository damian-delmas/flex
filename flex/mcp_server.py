...
def build_instructions() -> str:
    ...
    parts.extend([
        ...
        "",
        "DECISION TREE: Find X",
        "",
        "  Exact phrase, domain, filename, error, UUID?",
        "    → SQL: WHERE content LIKE '%term%'",
        "    → FTS5: SELECT * FROM content_fts WHERE content_fts MATCH 'term' ORDER BY bm25(content_fts)",
        "",
        "  Temporal or structural (when/how much/who)?",
        "    → SQL: COUNT(*), GROUP BY, WHERE created_at > ...",
        "",
        "  Conceptual / semantic (what about X)?",
        "    → vec_ops — plain cosine by default",
        "    → add tokens to modulate: diverse (broad), recent:N (decay), unlike:X (contrast)",
        "",
